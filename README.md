...
│   └── test/
│       ├── java/com/ustb/
│       │   └── PackageControllerTest.java              # 29 条测试用例
│       └── resources/
...
| 分页 | TC-18 | 分页参数验证 |
| 货架管理 | TC-19 ~ TC-23 | 列表查询、新增、重复新增（409）、删除、删除不存在（404） |
| 快递管理 | TC-24 ~ TC-28 | 列表查询、新增、重复新增（409）、删除、删除不存在（404） |
| 管理员取件 | TC-29 | 管理员路径取件（PUT /api/admin） |
